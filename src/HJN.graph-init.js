...
HJN.init.ChartRegist = function(chartName){
	...
	// 初期表示データを自動生成する
	HJN.init.CreateSampleTatLogAndChartShow() // #53
}
 ...
 * 
 * @param {String|Number}
 *            [json = HJN.init.CreateSampleTatLogJson(0)]
 *            シミュレーション条件JSONテキスト、もしくはサンプルJSON番号
 */
HJN.init.CreateSampleTatLogAndChartShow = function(json){ // #53
    "use strict";
    if (typeof(json) === "number") { // #59
        json = HJN.init.CreateSampleTatLogJson(json);
    } else{
 ...
 */
HJN.init.CreateSampleTatLog = function(jsonText){ // #53
    "use strict";
    var vSys = {};
    // parse
    var json = JSON.parse(jsonText)
    // virtual system と resources の設定
    var start = (json.start === undefined) ? new Date(1970,1,2) : HJN.util.S2D(json.start);
    ...
    vSys = HJN.util.VirtualSystem(start, end, json.resources);
    // model の取得
...
HJN.init.CreateSampleTatLog1 = function(){
    ...
    var eTat = [];       // 戻り値
    var x = new Date(), d= Math.floor(x.getTime()), y = 0.0, random = 0;
    for (var i = 0; i < num; i++) { // jsはミリ秒単位
        ...
        y  = Math.round( random*random*response * 1e+6 *
                (1 + (1.0 * Math.sin(2 * Math.PI * (i / num - 0.25)))) ) / 1e+6;
...
HJN.init.CreateSampleTatLog2 = function(){ // #53
    ...
    var onMode2 = { // オンライン取引2
            baseModel: {holds:["WEB","AP","DB"], tatMin: 2, tat:5},
            sequence: [ {hold:"",    tatMin:100,tat:500} ], // イベントシーケンス
            times: 60,   // イベントシーケンスの繰り返し回数
            thinkTimeMin: 500,  // イベントシーケンス終了時に再実行する場合の最小再開時間
            thinkTime: 1*sec    // イベントシーケンス終了時に再実行する場合の平均再開時間
        };
    var model3 = '' // バッチ取引3
        + '{ "baseModel":  {"holds": ["DB"], "tatMin": 2, "tat":5},'
        + '  "sequence": [{"hold":"TBL_B","tatMin":2000,"tat":3000}]}';
    ...
    var vSys = HJN.util.VirtualSystem(start, end);
 ...
 */
HJN.init.CreateSampleTatLogJson = function(n){ // #53
    "use strict";
...
HJN.util.FileReader = (function() {
	...
	function FileReader(arg){ 
        ...
        this.configId = "_config_" + "File"; // #59
		...
		// 名称と挙動の定義
		var env = "File";
		this["_config_" + env] = HJN.util.Config(env) // #59
        ...
        // Simulator Config用関数定義(radio用） #59
        env = "Simulator"
		...
		// Simulator Config設定画面定義 #59
        this["_config_" + env] = HJN.util.Config(env) // #59
        .label(null,"----- select simulate JSON --------").n()
        .name("S_SIMU")
            .radio("S_SIMU_000", null, "1 hour. online[200ms 5tps] batch[5sec evry3min]", false ,null, func_S_SIMU_000).n()
            .radio("S_SIMU_001", null, "for test", true ,null, func_S_SIMU_001).n()
        ;
	...
	FileReader.prototype.createGetterOfXY = function() {
		...
		function GetterOfXY(){ /* constructor */
				...
				var c = HJN.chart.fileReader;
				this.configId = "_config_" + "Filter"; // #59
	...
	FileReader.prototype.getConfigHtml = function(type) {
	    type = type || "File";
        return this["_config_" + type].getHtml(); // #59

	    if (type === "Simulator"){ // #59
	        return this._configSimulator.getHtml();
		} else if (type === "Filter"){
	            return this["_config_Filter"].getHtml();
		} else{	// "File"
			return this[this.configId].getHtml();
		}
	};
    /**
